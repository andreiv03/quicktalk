...
export async function POST(req: Request) {
	try {
		const { username, email, password, userAgent, ip } = await req.json();
		const formData: RegisterFormData = { username, email, password, userAgent, ip };

		const { data } = await axios.post<AuthResponse>("/auth/register", formData);
